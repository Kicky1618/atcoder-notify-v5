...
import { ScrapingState } from '../scraper/scrapingState';
...
export async function rebuildTasksTable(options: RebuildTasksTableOptions = {}) {
    if (rebuildingTasksTable) {
        ...
        await ScrapingState.set('tasks', 'skipped', { reason: 'already_running' });
        return;
    }

    rebuildingTasksTable = true;
    try {
        await ScrapingState.run('tasks', { contestIds: options.contestIds }, async () => {
            ...
            await ScrapingState.set('tasks', 'running', {
...
async function rebuildContestTasks(contestId: string) {
    await ScrapingState.set(`task:${contestId}`, 'running', { contestId });
    try {
        const db = Database.getDatabase();
        ...
        try {
            ...
            if (status === 403 || status === 404) {
                ...
                await ScrapingState.set(`task:${contestId}`, 'skipped', { contestId, status });
                return;
            }
            AtCoderScraper.logger?.error(`Failed to fetch tasks page for contest ${contestId}.`, { error });
            await ScrapingState.set(`task:${contestId}`, 'error', { contestId }, error);
            return;
        }
        ...
        if (response.status !== 200) {
            ...
            await ScrapingState.set(`task:${contestId}`, 'error', { contestId, status: response.status });
            return;
        }
        ...
        if (taskIds.size === 0) {
            ...
            await ScrapingState.set(`task:${contestId}`, 'success', { contestId, taskCount: 0, addedTaskCount: 0 });
            return;
        }
        ...
        if (missingTaskIds.length === 0) {
            await ScrapingState.set(`task:${contestId}`, 'success', {
                contestId,
        ...
        await ScrapingState.set(`task:${contestId}`, 'success', {
            contestId,
            taskCount: taskIds.size,
            addedTaskCount: missingTaskIds.length,
        });
    } catch (error) {
        await ScrapingState.set(`task:${contestId}`, 'error', { contestId }, error);
        throw error;
